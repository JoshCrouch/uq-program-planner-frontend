...
import type { ProgramComponent } from "../../consumables/ProgramClasses/ProgramComponents/ProgramComponent.ts";
...
<template>
  ...
  >
    <FloatLabel variant="in">
      <InputText id="dialog-program-title" v-model="programDialog.title" />
      <label for="dialog-program-title">Program Title</label>
    </FloatLabel>

    <FloatLabel variant="in">
      <InputText id="dialog-program-code" v-model="programDialog.code" />
      <label for="dialog-program-code">Program Code</label>
    </FloatLabel>

    <FloatLabel variant="in">
      <InputText id="dialog-program-year" v-model="programDialog.year" />
      <label for="dialog-program-year">Program Year</label>
    </FloatLabel>

    <FloatLabel variant="in">
      <InputText id="dialog-program-units" v-model="programDialog.units" />
      <label for="dialog-program-units">Program Units</label>
    </FloatLabel>
  </EditDialog>

  <!-- Dialog for adding program components -->
  <AddProgramComponentDialog
    :dialogVisible="addProgramComponentDialogVisible"
    :addCallback="
      (programComponent) =>
        controller.getProgram().addComponent(programComponent)
    "
    :closeCallback="closeAddProgramComponentDialog"
    :hasProgramElective="
      () => controller.getProgram()?.hasProgramElectiveComponent() ?? false
    "
    :hasGeneralElective="
      () => controller.getProgram()?.hasGeneralElectiveComponent() ?? false
    "
  />
  ...
  <div class="program-section" v-if="controller.getProgram()">
      ...
      <div class="program-description">
        ...
        <div class="program-units">
          <span class="units-number">{{
            controller.getProgram().getUnits()
          }}</span>
    ...
    <div>
      <draggable
        v-model="components"
        :item-key="(component: ProgramComponent) => component.getId()"
        class="course-grid"
        @end="onReorder"
        :ghost-class="'ghost-component'"
        :animation="300"
      >
        <template #item="{ element: component, attributes }">
          <div v-bind="attributes">
            <component
              :is="componentMap[component.getType()]"
              :model="component"
              :deleteCallback="() => deleteComponent(component.getId())"
            />
          </div>
        </template>
      </draggable>
      <AddButton
        :clickCallback="onAddButtonClick"
        text="Add Component"
      ></AddButton>
    </div>
  </div>
    ...
    .program-header-top {
      ...
      .edit-menu {
        ...
        .edit-button {
          ...
          --p-button-primary-active-border-color: var(
            --surface-light-color-alt
          );
