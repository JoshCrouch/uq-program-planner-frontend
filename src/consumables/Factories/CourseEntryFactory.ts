...
export function RegisterCourseEntry(type: string) {
  return function (constructor: any) {
    // Use 'any' to allow static property assignment
...
export class CourseEntryFactory {
   ...
   */
  public static registerCourseEntry(
    type: string,
    courseEntryClass: typeof CourseEntry,
  ): boolean {
   ...
   */
  public static async createCourseEntryFromJSON(
    json: any,
  ): Promise<CourseEntry> {
    ...
    if (!courseEntryClass) {
      throw new Error(
        `Course entry type "${courseEntryType}" is not registered.`,
      );
    }
  ...
  public static toJSON(courseEntry: CourseEntry): JSONCourseEntry {
    ...
    if (!courseEntryClass) {
      throw new Error(
        `Course entry type "${courseEntry.getType()}" is not registered.`,
      );
    }
