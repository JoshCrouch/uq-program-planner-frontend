...
export class ProgramFactory {
   ...
   */
  public static async buildProgramFromLoaded(
    loadedProgram: JSONProgram,
  ): Promise<Program> {
    ...
    for (const componentJson of loadedProgram.components) {
      const component: ProgramComponent =
        await ComponentFactory.createComponentFromJSON(componentJson);
  ...
  public static buildJSONFromProgram(program: Program): JSONProgram {
    ...
    for (const component of program.getComponents()) {
      jsonProgram.components.push(
        ComponentFactory.createJSONFromComponent(component),
      );
    }
