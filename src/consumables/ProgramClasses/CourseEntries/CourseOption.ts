...
export class CourseOption extends CourseEntry {
  ...
  public static async fromJSON(
    ...
    try {
      ...
      const optionOne: Course = await Course.fromCourseCode(
        componentJson.optionOne,
      );
      const optionTwo: Course = await Course.fromCourseCode(
        componentJson.optionTwo,
      );
      ...
      throw new Error(
        `Could not create CourseOption from JSON: ${err?.message}`,
      );
    }
  }
  ...
  public static async fromCourseCodes(
    optionOneCode: string,
    optionTwoCode: string,
  ): Promise<CourseOption> {
    ...
    try {
      ...
      throw new Error(
        `Could not create CourseOption from course codes: ${err?.message}`,
      );
    }
  }
