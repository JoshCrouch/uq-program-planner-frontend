...
import type {
  JSONCourseEntry,
  JSONProgramComponent,
} from "../../Factories/ProgramFactory.ts";
...
export class Section extends ProgramComponent {
  ...
  constructor(
    ...
    minUnits: number,
    maxUnits: number,
    courseGroups: Array<CourseEntry> = [],
  ) {
  ...
  static async fromJSON(componentJson: JSONProgramComponent): Promise<Section> {
    ...
    if (
      !componentJson.courseEntries ||
      !Array.isArray(componentJson.courseEntries)
    ) {
    ...
    for (const courseEntryJSON of componentJson.courseEntries) {
      const courseEntry: CourseEntry =
        await CourseEntryFactory.createCourseEntryFromJSON(
          courseEntryJSON as JSONCourseEntry,
        );
  ...
  static toJSON(section: Section): JSONProgramComponent {
    return {
      ...
      courseEntries: section
        .getCourseEntries()
        .map((courseEntry) => CourseEntryFactory.toJSON(courseEntry)),
    };
  }
