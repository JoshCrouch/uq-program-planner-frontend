...
export function useCategoryViewLogic(model: Category) {
  ...
  // Reactive list of sections for drag-and-drop (computed to stay in sync with the model)
  const sections = computed<Section[]>({
    get: () => model.getSections(),
    set: (val: Section[]) => model.setSections(val),
  });

  /**
   * Callback function for when the sections are reordered.
   */
  function onReorder() {
    // Update the model with the reordered sections
    toast.add({
      severity: "success",
      summary: "Sections Reordered",
      detail: "The sections have been reordered successfully.",
      life: 3000,
    });
  }
  ...
  function onDialogSave() {
    ...
    if (
      !categoryDialog.value.title ||
      !categoryDialog.value.minUnits ||
      !categoryDialog.value.maxUnits
    ) {
      toast.add({
        severity: "error",
        summary: "Error",
    ...
    if (
      isNaN(parseInt(categoryDialog.value.minUnits)) ||
      isNaN(parseInt(categoryDialog.value.maxUnits))
    ) {
      toast.add({
        severity: "error",
        summary: "Error",
    ...
    if (
      parseInt(categoryDialog.value.minUnits) >
      parseInt(categoryDialog.value.maxUnits)
    ) {
      toast.add({
        severity: "error",
        summary: "Error",
