...
export function useSectionViewLogic(model: Section) {
  ...
  // Reactive list of course entries for drag-and-drop (computed to stay in sync with the model)
  const entries = computed<CourseEntry[]>({
    get: () => model.getCourseEntries(),
    set: (val: CourseEntry[]) => model.setCourseEntries(val),
  });

  /**
   * Callback function for when the course entries are reordered.
   */
  function onReorder() {
    toast.add({
      severity: "success",
      summary: "Course Entries Reordered",
      detail: "The course entries have been reordered successfully.",
      life: 3000,
    });
  }
  ...
  function onEditDialogSave() {
    ...
    if (
      !sectionDialog.value.title ||
      !sectionDialog.value.minUnits ||
      !sectionDialog.value.maxUnits
    ) {
      toast.add({
        severity: "error",
        summary: "Error",
    ...
    if (
      isNaN(parseInt(sectionDialog.value.minUnits)) ||
      isNaN(parseInt(sectionDialog.value.maxUnits))
    ) {
      toast.add({
        severity: "error",
        summary: "Error",
    ...
    if (
      parseInt(sectionDialog.value.minUnits) >
      parseInt(sectionDialog.value.maxUnits)
    ) {
      toast.add({
        severity: "error",
        summary: "Error",
